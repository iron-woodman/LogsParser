...
namespace LogsParser
{
    static public class SFM
    {
        ...
        /// <summary>
        /// корневой каталог БД отфильтрованных логов
        /// </summary>
        static public string BD_FOLDER = Path.Combine(Application.StartupPath, "BD/");
        
        /// <summary>
        /// каталог БД отфильтрованных логов авторизации
        /// </summary>
        static public string AUTHORIZATION_BD_FOLDER = Path.Combine(Application.StartupPath, $"BD/AUTHORIZATION");

        /// <summary>
        /// каталог БД отфильтрованных логов событий поднятия предметов персонажем
        /// </summary>
        static public string CHARACTER_PICK_BD_FOLDER = Path.Combine(Application.StartupPath, $"BD/CHARACTER_PICK");

        /// <summary>
        /// каталог БД отфильтрованных логов событий поднятия предметов монстром
        /// </summary>
        static public string MONSTER_PICK_BD_FOLDER = Path.Combine(Application.StartupPath, $"BD/MONSTER_PICK");

        /// <summary>
        /// каталог БД отфильтрованных логов событий продажи
        /// </summary>
        static public string SALE_BD_FOLDER = Path.Combine(Application.StartupPath, $"BD/SALE");

        /// <summary>
        /// каталог БД отфильтрованных логов событий покупки
        /// </summary>
        static public string BUY_BD_FOLDER = Path.Combine(Application.StartupPath, $"BD/BUY");

        /// <summary>
        /// каталог БД отфильтрованных логов событий торговли
        /// </summary>
        static public string TRADE_BD_FOLDER = Path.Combine(Application.StartupPath, $"BD/TRADE");

        /// <summary>
        /// каталог БД отфильтрованных логов событий удалдения
        /// </summary>
        static public string DELETE_BD_FOLDER = Path.Combine(Application.StartupPath, $"BD/DELETE");

        /// <summary>
        /// каталог БД отфильтрованных логов событий разбора
        /// </summary>
        static public string DECOMPOSE_BD_FOLDER = Path.Combine(Application.StartupPath, $"BD/DECOMPOSE");
        #endregion

        #region OnApplicationStartup
        static public void OnApplicationStartup()
        {
            CreateBDFolderIFNotExist(BD_FOLDER);
            CreateBDFolderIFNotExist(AUTHORIZATION_BD_FOLDER);
            CreateBDFolderIFNotExist(CHARACTER_PICK_BD_FOLDER);
            CreateBDFolderIFNotExist(MONSTER_PICK_BD_FOLDER);
            CreateBDFolderIFNotExist(SALE_BD_FOLDER);
            CreateBDFolderIFNotExist(BUY_BD_FOLDER);
            CreateBDFolderIFNotExist(TRADE_BD_FOLDER);
            CreateBDFolderIFNotExist(DELETE_BD_FOLDER);
            CreateBDFolderIFNotExist(DECOMPOSE_BD_FOLDER);
        ...
        static public List<ListInterface> ReadFile(string file, DataGridView pDgv)
        {
            ...
            using (StreamReader sr = new StreamReader(BD_FOLDER + file, Encoding.Default, true))
            {
        ...
        static private void CreateBDFolderIFNotExist(string folder)
        {
        ...
        {
            using (TextWriter tr = new StreamWriter(BD_FOLDER + szFile, false))
            {
        ...
        {
            using (TextWriter tr = new StreamWriter(BD_FOLDER + "characters.txt", false))
            {
        ...
        {
            using (TextWriter tr = new StreamWriter(BD_FOLDER + "warplist.txt", false))
            {
        ...
        static public void ImportList(string szFilename, string szType, DataGridView dgv)
        {
            try
            {
                if (File.Exists(BD_FOLDER + szType))
                {
                    File.Delete(BD_FOLDER + szType);
                }
                File.Copy(szFilename, BD_FOLDER + szType);
        ...
        static public void ExportList(string szLocation, string szType)
        {
            try
            {
                ...
                File.Copy(BD_FOLDER + szType, szLocation);
            }
            catch (Exception ex)
            {
